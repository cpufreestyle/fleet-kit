#!/usr/bin/env python3
"""把 Qwen Cloud（千问海外托管 API）模型注入 Codex 的 cc-switch 模型目录（slug: qwen/<model>）。"""
import json, os, sys
from pathlib import Path

CATALOG = Path(os.path.expanduser("~/.codex/cc-switch-model-catalog.json"))

# qwen3.8-flash 是 Qwen4 架构预览（Flash-Next）的生产版：1M 上下文，65K 摘要窗口。
BASE_CTX = {
    "qwen3.8-flash": 1000000,
    "qwen3.8-max": 1000000,
}


def main() -> int:
    bridge = os.environ.get("QWEN_BRIDGE", "http://127.0.0.1:8798")
    key = os.environ.get("QWEN2CODEX_KEY", "")
    import urllib.request
    req = urllib.request.Request(f"{bridge}/v1/models")
    if key:
        req.add_header("Authorization", f"Bearer {key}")
    with urllib.request.urlopen(req, timeout=60) as resp:
        data = json.loads(resp.read().decode())

    models = data.get("data") or []
    if not models:
        print("bridge returned no models")
        return 1

    doc = json.loads(CATALOG.read_text(encoding="utf-8"))
    entries = doc["models"] if isinstance(doc, dict) else doc
    existing = {(m.get("slug") or m.get("id")) for m in entries if isinstance(m, dict)}
    added = 0
    for m in models:
        mid = m["id"]
        mid = mid.removeprefix("qwen/")
        slug = f"qwen/{mid}"
        if slug in existing:
            continue
        ctx = BASE_CTX.get(mid, 1000000)
        entries.append({
            "slug": slug,
            "display_name": f"Qwen {mid}",
            "description": f"Qwen Cloud 海外托管 {mid}（Qwen4 架构生产版，免费额度可用；经 qwen2codex 本地桥）",
            "max_context_window": ctx,
            "context_window": ctx,
            "effective_context_window_percent": 95,
            "default_reasoning_level": "medium",
            "default_reasoning_summary": "auto",
            "support_verbosity": True,
            "input_modalities": ["text", "image", "video"],
            "priority": 50,
        })
        added += 1
    if isinstance(doc, dict):
        doc["models"] = entries
        CATALOG.write_text(json.dumps(doc, ensure_ascii=False, indent=2), encoding="utf-8")
    else:
        CATALOG.write_text(json.dumps(entries, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"models from bridge: {len(models)}; added {added}; catalog total: {len(entries)}")
    return 0


if __name__ == "__main__":
    sys.exit(main())
