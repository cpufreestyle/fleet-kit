#!/usr/bin/env python3
"""把 Qoder 模型注入 Codex 的 cc-switch 模型目录（slug: qoder/<model>）。"""
...
def main() -> int:
    bridge = os.environ.get("QODER_BRIDGE", "http://127.0.0.1:8789")
    key = os.environ.get("QODER2CODEX_KEY", "")
    ...
    for m in models:
        ...
        entries.append({
            "slug": slug,
            "display_name": f"Qoder {mid}",
            "description": f"Qoder CN 订阅模型 {mid}（经 qoder2codex 本地桥）",
